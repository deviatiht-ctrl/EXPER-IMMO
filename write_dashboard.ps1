...
async function loadStats() {
    try {
        ...
        const s = data || {};
        setEl('stat-proprietes',    s.total_proprietes     ?? 0);
        setEl('stat-proprietaires', s.total_proprietaires  ?? 0);
        setEl('stat-locataires',    s.total_locataires     ?? 0);
        setEl('stat-contrats',      s.contrats_actifs      ?? 0);
        setEl('stat-paiements',     '$' + fmtNum(s.revenus_ce_mois ?? 0));
...
async function loadPaiementsRetard() {
    ...
    try {
        ...
        c.innerHTML = data.map(p =>
            ...
            '<h4>$' + fmtNum(p.montant_total || 0) + ' — ' + esc(p.locataire?.user?.full_name || 'N/A') + '</h4>' +
...
'@
[System.IO.File]::WriteAllText('c:\Users\HACKER\Pictures\IMMOBILIER TEST 2\js\admin\dashboard.js', $js, [System.Text.Encoding]::UTF8)
Write-Host "Written. Lines:" (Get-Content 'c:\Users\HACKER\Pictures\IMMOBILIER TEST 2\js\admin\dashboard.js').Count
