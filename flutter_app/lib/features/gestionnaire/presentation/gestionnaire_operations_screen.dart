...
class GestionnaireOperationsScreen extends ConsumerStatefulWidget {
  const GestionnaireOperationsScreen({super.key});

  @override
  ConsumerState<GestionnaireOperationsScreen> createState() => _State();
}

class _State extends ConsumerState<GestionnaireOperationsScreen> {
  String _search = '';

  @override
  Widget build(BuildContext context) {
    final dataAsync = ref.watch(gestionnaireOperationsProvider);
    ...
    return Column(
      children: [
        Padding(
          ...
          child: Row(
            children: [
              Expanded(child: Text('Mes Opérations', style: Theme.of(context).textTheme.headlineLarge)),
              IconButton(icon: const Icon(Icons.refresh, size: 20), onPressed: () => ref.invalidate(gestionnaireOperationsProvider)),
            ],
          ),
        ),
        Padding(
        ...
        Expanded(
          child: dataAsync.when(
            ...
            error: (e, _) => ErrorRetryWidget(message: e.toString(), onRetry: () => ref.invalidate(gestionnaireOperationsProvider)),
            data: (list) {
              ...
              return RefreshIndicator(
                onRefresh: () async => ref.invalidate(gestionnaireOperationsProvider),
  ...
  Widget _buildCard(Map<String, dynamic> o) {
    ...
    final published = o['publie_portail'] == true;
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const Spacer(),
              if (published) const Row(
                children: [
                  Icon(Icons.visibility, size: 12, color: AppColors.adminInfo),
                  SizedBox(width: 4),
                  Text('Publié', style: TextStyle(fontSize: 11, color: AppColors.adminInfo)),
                ],
              ),
            ],
          ),
