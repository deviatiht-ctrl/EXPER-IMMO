...
class GestionnaireContratsScreen extends ConsumerStatefulWidget {
  const GestionnaireContratsScreen({super.key});

  @override
  ConsumerState<GestionnaireContratsScreen> createState() => _State();
}

class _State extends ConsumerState<GestionnaireContratsScreen> {
  String _search = '';

  @override
  Widget build(BuildContext context) {
    final dataAsync = ref.watch(gestionnaireContratsProvider);
      ...
      children: [
        Padding(
          ...
          child: Row(
            children: [
              ...
              IconButton(icon: const Icon(Icons.refresh, size: 20), onPressed: () => ref.invalidate(gestionnaireContratsProvider)),
            ],
          ),
        ),
        Padding(
        ...
        Expanded(
          child: dataAsync.when(
            ...
            error: (e, _) => ErrorRetryWidget(message: e.toString(), onRetry: () => ref.invalidate(gestionnaireContratsProvider)),
            data: (list) {
              ...
              return RefreshIndicator(
                onRefresh: () async => ref.invalidate(gestionnaireContratsProvider),
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const SizedBox(width: 6),
              Text('${formatDate(c['date_debut']?.toString())} à ${formatDate(c['date_fin']?.toString())}', style: const TextStyle(fontSize: 11, color: AppColors.textMuted)),
            ],
          ),
        ],
      ),
