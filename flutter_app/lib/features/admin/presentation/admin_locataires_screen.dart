...
class AdminLocatairesScreen extends ConsumerStatefulWidget {
  const AdminLocatairesScreen({super.key});

  @override
  ConsumerState<AdminLocatairesScreen> createState() => _AdminLocatairesScreenState();
}

class _AdminLocatairesScreenState extends ConsumerState<AdminLocatairesScreen> {
  String _search = '';

  @override
  Widget build(BuildContext context) {
    final dataAsync = ref.watch(adminLocatairesProvider);
    ...
    return Column(
      children: [
        Padding(
          ...
          child: Row(
            children: [
              Expanded(child: Text('Locataires', style: Theme.of(context).textTheme.headlineLarge)),
              IconButton(icon: const Icon(Icons.refresh, size: 20), onPressed: () => ref.invalidate(adminLocatairesProvider)),
            ],
          ),
        ),
        Padding(
        ...
        Expanded(
          child: dataAsync.when(
            ...
            error: (e, _) => ErrorRetryWidget(message: e.toString(), onRetry: () => ref.invalidate(adminLocatairesProvider)),
            data: (list) {
              ...
              return RefreshIndicator(
                onRefresh: () async => ref.invalidate(adminLocatairesProvider),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          if (phone.isNotEmpty) IconButton(icon: const Icon(Icons.phone_outlined, size: 18, color: AppColors.locataireColor), onPressed: () {}),
        ],
      ),
    );
  }
